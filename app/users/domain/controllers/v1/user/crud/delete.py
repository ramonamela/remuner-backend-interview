...
class DeleteUserControllerV1:
    def __init__(self, user_bo_persistence_service, integration_bo_persistence_service):
        ...
        self.integration_bo_persistence_services: IntegrationBOPersistenceInterface = integration_bo_persistence_service

    async def __call__(self, user_id: int):
        integration_bos = await self.integration_bo_persistence_services.get_integrations_for_user(user_id=user_id)
